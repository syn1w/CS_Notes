...
编译器有一个**前端** ，用于处理源语言 。它还有一个**后端** ，用于处理目标语言。为将前端和后端连接起来 ，编译器有一种形式化的结构 ，它用一种**中间形式**来表示程序 。中间形式的语言很大程度上强立于源语言和目标语言 。 为改进转换 ，编译器通常包括一个**优化器**， 来分析并重写中间形式。
...
![Thompson](../../imgs/compiler/2_4.png)



一个例子，见图 2-5

![NFA_example](../../imgs/compiler/2_5.png)
...
![DFA_example](../../imgs/compiler/2_6c.png)
...
![hopcroft_example](../../imgs/compiler/2_7.png)
...
![table-driver lexer](../../imgs/compiler/2_8.png)
...
![parsing tree](../../imgs/compiler/3_1.png)
...
构建自顶向下语法分析器有两种技术：手工编写递归下降分析器和生成的LL(1) 语法分析器。
